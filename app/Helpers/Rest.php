...
class Rest
{
    ...
    {
        $this->rest_server = data_get($config, 'server');
        $this->timeout = data_get($config, 'timeout', 30);

    }

    public function post($uri, $request, $headers = array())
    {
        $ch = curl_init($this->rest_server . '/' . $uri);
        ...
        if (curl_errno($ch)) {
            throw new RestClientException(isset(self::$error_codes[curl_errno($ch)]) ? self::$error_codes[curl_errno($ch)] : "UNKNOWN_ERROR");
        }
    ...
    {
        $result['info']['http_code'] = $this->_headers['http_code'];
        $result['response_string'] = $this->_body;
        return $result;
    }
